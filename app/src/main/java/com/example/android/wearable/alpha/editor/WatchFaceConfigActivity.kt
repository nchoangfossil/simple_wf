...
import com.example.android.wearable.alpha.data.watchface.BackGroundStyle
...
//        binding.minuteHandLengthSlider.value = userStylesAndPreview.minuteHandLength
    ...
    fun onClickBackGroundPickerButton(view: View) {
        val backgroundList = enumValues<BackGroundStyle>()
        val newBackGroundStyle: BackGroundStyle = backgroundList.random()
        stateHolder.setBackGroundStyle(newBackGroundStyle.id)
    }
    ...
    fun onClickMediumStyleButton(view: View) {
        stateHolder.setDigitalStyle(digitalStyleId = DigitalStyle.MEDIUM.id)
    }

    fun onClickLargeStyleButton(view: View) {
        stateHolder.setDigitalStyle(digitalStyleId = DigitalStyle.LARGE.id)
    }

    fun onClickSmallStyleButton(view: View) {
        stateHolder.setDigitalStyle(digitalStyleId = DigitalStyle.SMALL.id)
    }

    fun onClickChangeModeButton(view: View) {
        watchFaceMode = if (this.watchFaceMode == WatchFaceMode.ANALOG){
            WatchFaceMode.DIGITAL
        } else {
            WatchFaceMode.ANALOG
        }
        stateHolder.setWatchFaceMode(watchFaceModeInput = watchFaceMode)
    }
